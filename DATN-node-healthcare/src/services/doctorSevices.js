            ...
            if (!nameDoctor) {
                ...
                const doctor = await db.User.findAll({
                  where: {
                    roleId: 'R2',
                    [Op.or]: [
                        // Tìm theo họ, tên, hoặc họ tên ghép lại
                        { firstName: { [Op.like]: `%${nameDoctor}%` } },
                        { lastName: { [Op.like]: `%${nameDoctor}%` } },
                        db.sequelize.literal(`CONCAT(firstName, ' ', lastName) LIKE '%${nameDoctor}%'`)
                    ]
                  },
                  attributes: {
